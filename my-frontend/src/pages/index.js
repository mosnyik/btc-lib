...
export default function Home() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const storedBlockDetails = localStorage.getItem("blockDetails");
      setBlockDetails(storedBlockDetails ? JSON.parse(storedBlockDetails) : {});
    }
  }, []);
 ...
 const reserveSlot = async () => {
  ...
  const removeWitnessData = (txHex) => {
   ...
   const mintNFT = async () => {
    ...
    const contractName = "verifytx"; // Replace with your contract name
  ...
  const getButtonState = () => {
    ...
    return {
      ...
      onClick: reserveSlot,
      disabled: false,
      instructions: "Step 1: Reserve your slot by sending 100 sats",
    };
  };

  return (
    <main className="flex flex-col items-center justify-center min-h-screen p-24 bg-purple-300">
      <h1 className="text-6xl font-bold text-center text-white">Premium Movies</h1>
      {!userData.profile ? (
        <button
